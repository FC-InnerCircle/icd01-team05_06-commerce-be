            ...
            .andDo(
                document(
                    ...
                    responseFields(
                        ...
                        fieldWithPath("depth").type(JsonFieldType.NUMBER).description("뎁스"),
                        fieldWithPath("childCategories").type(JsonFieldType.ARRAY).description("자식 카테고리").optional(),
                        fieldWithPath("childCategories[].id").type(JsonFieldType.NUMBER).description("자식 카테고리 id"),
                        fieldWithPath("childCategories[].parentId").type(JsonFieldType.NUMBER).description("자식 카테고리의 부모 카테고리 id"),
                        fieldWithPath("childCategories[].name").type(JsonFieldType.STRING).description("자식 카테고리 이름"),
                        fieldWithPath("childCategories[].depth").type(JsonFieldType.NUMBER).description("자식 카테고리의 뎁스"),
                        fieldWithPath("childCategories[].childCategories").type(JsonFieldType.ARRAY).description("더 하위의 자식 카테고리 리스트 (null일 수 있음)").optional(),
                        fieldWithPath("childCategories[].childCategories[].id").type(JsonFieldType.NUMBER).description("하위 자식 카테고리 id").optional(),
                        fieldWithPath("childCategories[].childCategories[].parentId").type(JsonFieldType.NUMBER).description("하위 자식 카테고리의 부모 카테고리 id").optional(),
                        fieldWithPath("childCategories[].childCategories[].name").type(JsonFieldType.STRING).description("하위 자식 카테고리 이름").optional(),
                        fieldWithPath("childCategories[].childCategories[].depth").type(JsonFieldType.NUMBER).description("하위 자식 카테고리의 뎁스").optional(),
                        fieldWithPath("childCategories[].childCategories[].childCategories").type(JsonFieldType.ARRAY).description("더 하위의 자식 카테고리 리스트 (null일 수 있음)").optional()
                    )
                )
            )
    }
        ...
        .andDo(
            document(
                ...
                responseFields(
                    ...
                    fieldWithPath("[].discountedPrice").type(JsonFieldType.NUMBER).description("할인 가격"),
                    ...
                    fieldWithPath("[].pages").type(JsonFieldType.NUMBER).description("페이지 수"),
                    fieldWithPath("[].coverImage").type(JsonFieldType.STRING).description("커버 이미지"),
                    fieldWithPath("[].previewLink").type(JsonFieldType.STRING).description("미리보기 링크"),
                    fieldWithPath("[].stockQuantity").type(JsonFieldType.NUMBER).description("남은 수량"),
                    fieldWithPath("[].rating").type(JsonFieldType.NUMBER).description("별점"),
                    fieldWithPath("[].status").type(JsonFieldType.STRING).description("상태"),

                    fieldWithPath("[].category").type(JsonFieldType.OBJECT).description("카테고리"),
                    ...
                    fieldWithPath("[].category.parentCategory").type(JsonFieldType.OBJECT).description("부모 카테고리"),
                    fieldWithPath("[].category.parentCategory.id").type(JsonFieldType.NUMBER).description("부모 카테고리 id"),
                    fieldWithPath("[].category.parentCategory.name").type(JsonFieldType.STRING).description("부모 카테고리 이름"),
                )
            )
        )
    }
